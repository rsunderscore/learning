...
- "**out-of-core learning** which allows us to work with such large datasets by fitting the classifier incrementally on smaller batches of a dataset.Text"  (pg 191)
  - requires models with a partial_fit function (e.g. SGDClassifier)
...
## topic modeling = "broad task of assigning topics to unlabeled text documents."  (pg 194)
...
- "Latent Dirichlet Allocation (LDA). However, note that while Latent Dirichlet Allocation is often abbreviated as LDA, it is not to be confused with linear discriminant analysis,"  (pg 194)
  ...
  - "probabilistic (vs frequentist) model that tries to find groups of words that appear frequently together"  (pg 195)
...
## Flask web application for mdoel (pg 204)
...
- "cost function in Adaline is the sum of squared errors (SSE), which is identical to the cost function that we use for OLS... OLS regression can be understood as Adaline without the unit step function so that we obtain continuous target values"  (pg 224)
...
- regression equation using matrices aka the  ***normal equation,. may be too expensive too invert the matrix for large datasets***  (pg 227)
...
- "1-hot vector representation allows us to tackle classification tasks with an arbitrary number of unique class labels present in the training"  (pg 265)
...
- classifying digits (MNIST)
  ...
  - dataset download via "scikit-learn's new fetch_openml"  (pg 272)
...
- "efficient and convenient method to save multidimensional arrays to disk is NumPy's savez function"  (pg 271)
  - **name of the function** "np.savez_compressed"  (pg 271)
...
- "performance-enhancing tricks 
  - "adaptive learning rates, more sophisticated SGD-based optimization algorithms, batch normalization, and dropout"  (pg 279)
...
- Keras - "simplifying API ... make the construction of common deep learning models even more convenient"  (pg 286)
...
- activation functions
  ...
  - "hyperbolic tangent (commonly known as tanh), which can be interpreted as a rescaled version of the logistic function"  (pg 317)
    ...
    - "vanishing gradient problem of tanh and logistic activations ... derivative of activations with respect to the net input diminishes as z becomes large"  (pg 318)
